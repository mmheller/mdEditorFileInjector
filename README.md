mdEditorFileInjector Readme
Author: Matt Heller, Great Northern Landscape Conservation Cooperative/U.S. Fish and Wildlife Service
Date: 11/28/2017, updated 11/14/2018
...
Option 7: Finds the Sciencebase ID from metadata|resourceInfo|citation|identifier where the namespace is gov.sciencebase.catalog.  Then a distributed links object is injected based on the found Sciencebase ID and some boiler plate content.  The script is hardcoded to add the LLC Data Steward contact id edc6b779-3352-4a81-8430-76bcce1bfcb3 (example: -C C:\Temp\Demo -T False -O 7)
...
Option 3: will utilize the find file (-f) and replace any field name (key name) found (not the value).
